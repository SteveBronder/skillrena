...
That's it. Running `$activating-memories` to bootstrap/load your project memories adds about ~7.6k tokens in total. Still leaves plenty of room for actual work.
...
So ~7.6k tokens and your agent knows enough about your project to start working. Not great, but not terrible.
...
### `$activating-memories`: Bootstrap and load project memory

When entering a project, call `$activating-memories` to bootstrap repo-local memory docs under `agent-docs/` and create **project-local skills** under `.{AGENT_NAME}/skills/`.

```
$activating-memories
```

After onboarding, you typically use the project-local skills:
- `$memories We will be working in <languages>`
- `$compress-memories <languages>` (when memories exceed the 500-line budget)
- `$write-memory <what to remember>`

**For the agents reading this**: `{AGENT_NAME}` in the skill files becomes `claude` or `codex` depending on which CLI is running. So `.{AGENT_NAME}/skills/` turns into `.claude/skills/` for Claude Code.
...
Diary entries live in:
- `agent-docs/diary/`

After a few entries, you can consolidate them and push durable items into `agent-docs/<lang>/*.md` via `$write-memory`.
